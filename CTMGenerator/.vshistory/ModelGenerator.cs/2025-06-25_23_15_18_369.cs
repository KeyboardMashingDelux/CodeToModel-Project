...
")}";
        ...
        private static (string? uri, string? filename) GetMetadata(IAssemblySymbol assembly) {
            if (assembly == null)
                return (null, null);
            ...
            var metadataAttribute = Utilities.GetAttributeByName(assembly.GetAttributes(), nameof(ModelMetadataAttribute));
            if (metadataAttribute == null) {
                return (null, null);
            }

            var ca = metadataAttribute.ConstructorArguments;
            uri = ca[0].Value?.ToString();
            filename = ca[1].Value?.ToString();
