...
namespace CTMGenerator {
    ...
    public class ModelGenerator : IIncrementalGenerator {
        ...
        private static void GenerateCode(SourceProductionContext context,
            ...
            Dictionary<string, ModelBuilder> models = new Dictionary<string, ModelBuilder>();
...
//        }
        ...
        private static List<(string uri, string filename)> GetMetadata(IAssemblySymbol assembly) {
            ...
            List<AttributeData> metadataAttribute = Utilities.GetAttributesByName(assembly.GetAttributes(), nameof(ModelMetadataAttribute));
            if (metadataAttribute.IsNullOrEmpty()) {
                return [];
            }

            List<(string uri, string filename)> metadata = []; 
            foreach (var attribute in metadataAttribute) {
