...
namespace CTMGenerator {
    ...
    public class ModelGenerator : IIncrementalGenerator {
        ...
        private string CompilerGeneratedFilesOutputPath = "";

        public void Initialize(IncrementalGeneratorInitializationContext context) {
            Debugger.Launch();

            var tsp = context.AdditionalTextsProvider
                .Combine(context.AnalyzerConfigOptionsProvider)
                .Select((provider, ct) => 
                    provider.Right.GetOptions(provider.Left).TryGetValue("build_property.CompilerGeneratedFilesOutputPath", out var compilerGeneratedFilesOutputPath) ? compilerGeneratedFilesOutputPath : "NOT LOADED");

            Debug.WriteLine("#####------##########" + tsp + " <> " + tsp.Collect());
...
{(ns is null ? null : $@"namespace {ns} {{")}
   public partial class {className} : {name} {(string.IsNullOrWhiteSpace(inheritance) ? null : ", " + inheritance)} {{
...
")}";
            ...
            ModelBuilder.CreateModel(elements);
        }
        ...
        private static string GetInheritance(ITypeSymbol element) {
            ...
            foreach (var interfaceSymbol in interfaces) {
                ...
                if (interfaceAttributes.Any(a => IsLibAttributeClass(a.AttributeClass, GeneratorResources.ModelInterafaceAttributeName))) {
                    inheritance.Add(interfaceSymbol.ToDisplayString().Substring(1));
                }
            }
