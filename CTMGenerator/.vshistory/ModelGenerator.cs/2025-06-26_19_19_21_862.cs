    ...
    public class ModelGenerator : IIncrementalGenerator {
        ...
        private static bool IsFromCorrectLib(ISymbol type) {
            return type.GetAttributes()
                       .Any(a => Utilities.IsLibAttributeClass(a.AttributeClass, GeneratorResources.ModelInterafaceAttributeName));
        }
        ...
        private static void GenerateCode(SourceProductionContext context,
            ...
            List<(string uri, string filename)> metadata = GetMetadata(providerData.assembly);
            Dictionary<string, ModelBuilder> models = [];
            foreach (var (uri, filename) in metadata) {
                ModelBuilder mb = new();
                mb.Initalize(uri, filename);
                // TODO Make Dynamic
                models.Add("CodeToModel.Example", mb);
            }
...
//        }
        ...
        private static string GetInheritance(ITypeSymbol element) {
            ...
            foreach (var interfaceSymbol in interfaces) {
                ...
                if (interfaceAttributes.Any(a => Utilities.IsLibAttributeClass(a.AttributeClass, GeneratorResources.ModelInterafaceAttributeName))) {
