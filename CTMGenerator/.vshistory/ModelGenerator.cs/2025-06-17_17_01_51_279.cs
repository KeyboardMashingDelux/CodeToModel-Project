        ...
        private static void GenerateModel(SourceProductionContext context, ImmutableArray<ITypeSymbol?> elements) {
            ...
            foreach (var element in elements) {
                ...
                foreach (var member in members) {
                    ...
                    var visibility = member.DeclaredAccessibility.ToString().ToLower();
                    var memberName = member.Name;
                    var varName = $"_{memberName.ToLower()}";
                    //var type = member.GetType().ToString();
                    // -> Microsoft.CodeAnalysis.CSharp.Symbols.PublicModel.PropertySymbol for the declaration
                    // -> Microsoft.CodeAnalysis.CSharp.Symbols.PublicModel.MethodSymbol for the type of methods (get/set)
                    var cn = member.ToString();

                    classVars += $"{visibility} {cn} {varName};\n";
                    classMembers += $"{visibility} {cn} {memberName} {{ get; }}\n";
                }
