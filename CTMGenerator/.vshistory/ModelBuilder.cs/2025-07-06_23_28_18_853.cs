...
namespace CTMGenerator {
    ...
    public class ModelBuilder {
        ...
        private string? FullName, Name, AmbientName, Prefix, Suffix, OutputPath;

        private Compilation GeneratorCompilation;

        /// Key = Ref Class name, Value Type to add reference to
        private Dictionary<string, IReference> RefTypeInfos;
        ...
        private void AddClass(ITypeSymbol element) {
            ...
            var (propertyReferences, propertyAttributes, refsForTypes) = ModelBuilderHelper.ConvertProperties(properties, GeneratorCompilation);
        ...
        public void CreateReferences() {
            foreach (var refInfo in new Dictionary<string, IReference>(RefTypeInfos)) {
                string refName = refInfo.Key;
                IReference refAddType = refInfo.Value;
