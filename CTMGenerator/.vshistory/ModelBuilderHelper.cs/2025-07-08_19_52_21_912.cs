...
namespace CTMGenerator {
    public class ModelBuilderHelper {
        ...
        public static (List<IReference> references, List<IAttribute> attributes, Dictionary<string, IReference> refTypeInfos) ConvertProperties(List<IPropertySymbol> properties, Compilation compilation) {
            List<IReference> references = [];
            List<IAttribute> attributes = [];
            Dictionary<string, IReference> opposites = [];
            Dictionary<string, IReference> refTypeInfos = [];
            foreach (var property in properties) {
                ...
                if (IsPrimitive(specialType)) {
                    ...
                    Attribute attribute = new() {
                        ...
                        Type = new PrimitiveType() { SystemType = specialType.ToString() }.GetClass()
                    };
                }
                else {
                    ImmutableArray<AttributeData> propertyAttributes = property.GetAttributes();
                    ...
                    if (typeArgument != null) {
                        if (IsPrimitive(typeArgument.SpecialType)) {
                            ...
                            // Add none primitive
                            refTypeInfos.Add(typeArgument.Name, reference);
                        }
                    }
                    else {
                        if (IsPrimitive(type.SpecialType)) {
                            ...
                            // Add none primitive
                            refTypeInfos.Add(type.Name, reference);
                        }
                    }
