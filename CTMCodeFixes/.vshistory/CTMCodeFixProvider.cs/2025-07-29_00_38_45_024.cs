...
namespace CTMCodeFixes {
    ...
    public class CTMCodeFixProvider : CodeFixProvider {
        ...
        private static async Task<Document> AddPartialKeyword(CodeFixContext context,
            ...
            InterfaceDeclarationSyntax? interfaceDeclaration = FindInterfaceDeclaration(makePartial, root);

            if (interfaceDeclaration == null)
                return context.Document;

            var partial = SyntaxFactory.Token(SyntaxKind.PartialKeyword);
            var newDeclaration = interfaceDeclaration.AddModifiers(partial);
            var newRoot = root.ReplaceNode(interfaceDeclaration, newDeclaration);
